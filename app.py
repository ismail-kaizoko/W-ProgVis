...
class PieWidgetData(db.Model):
    ...
    unit_mode = db.Column(db.String(20), nullable=False, default="numbers")
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
def ensure_schema_updates():
    ...
    if "pie_widget_data" in table_names:
        column_names = {column["name"] for column in inspector.get_columns("pie_widget_data")}
        if "unit_mode" not in column_names:
            with db.engine.begin() as connection:
                connection.exec_driver_sql(
                    "ALTER TABLE pie_widget_data ADD COLUMN unit_mode VARCHAR(20) NOT NULL DEFAULT 'numbers'"
                )
...
def normalize_pie_mode(raw_mode):
    mode = (raw_mode or "numbers").strip().lower()
    if mode not in {"numbers", "minutes"}:
        raise ValueError("Please choose either numbers or minutes for the pie chart.")
    return mode
...
def build_pie_plot(values):
    total_tracked = round(sum(values), 2)
    wasted_hours = round(max(0, 24 - total_tracked), 2)
...
def serialize_pie_widget(widget):
    ...
    today_entries = get_today_pie_entries(widget, len(categories))
    plot = build_pie_plot(today_entries)
    return {
        **widget.to_dict(),
...
def create_widget():
    ...
    if widget_type == "radar":
        ...
        try:
            ...
            unit_mode = normalize_pie_mode(data.get("unit_mode"))
        except ValueError as exc:
            return jsonify({"error": str(exc)}), 400
        ...
        pie_data = PieWidgetData(widget_id=widget.id, unit_mode=unit_mode)
...
def update_pie_entry(widget_id):
    ...
    if not isinstance(raw_hours, list) or len(raw_hours) != len(categories):
        return jsonify({"error": "Please provide one value for each activity."}), 400
    ...
    for value in raw_hours:
        try:
            ...
            return jsonify({"error": "Please enter valid values for each activity."}), 400

        if parsed_value < 0:
            return jsonify({"error": "Activity values cannot be negative."}), 400

        parsed_hours.append(round(parsed_value, 2))

    if sum(parsed_hours) > 24:
        return jsonify({"error": "Tracked time cannot exceed 24 hours for one day."}), 400
...
@app.route("/widgets/<int:widget_id>/pie/category", methods=["POST"])
@login_required
def add_pie_category(widget_id):
    widget = DashboardWidget.query.filter_by(
        id=widget_id, user_id=session["user_id"], widget_type="pie"
    ).first_or_404()
    ...
    if not raw_category:
        return jsonify({"error": "Please enter a new activity name."}), 400

    categories = widget.pie_data.get_categories()
    existing_names = {category.lower() for category in categories}
    lowered = raw_category.lower()

    if lowered == "wasted":
        return jsonify({"error": '"Wasted" is calculated automatically and cannot be added.'}), 400
    if lowered in existing_names:
        return jsonify({"error": "This activity already exists in the chart."}), 400

    categories.append(raw_category)
    widget.pie_data.set_categories(categories)
    db.session.commit()
