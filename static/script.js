...
function updateHoursGraph(data) {
  if (data.length === 0) {
    document.getElementById("hours-graph").innerHTML = "<p style='text-align: center; color: #666;'>No data yet. Add your first entry above!</p>";
    return;
  }
  ...
  Plotly.newPlot('hours-graph', [trace1, trace2], layout, { responsive: true });
}
...
async function addHoursEntry() {
  const workHours = parseFloat(document.getElementById('work-hours-input').value);
  const studyHours = parseFloat(document.getElementById('study-hours-input').value);

  if (isNaN(workHours) || isNaN(studyHours) || workHours < 0 || studyHours < 0) {
